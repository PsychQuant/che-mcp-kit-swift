...
|---|---|
| `SelfUpdate` | `--self-update`: fetch the latest GitHub release, check its SHA-256 companion, verify the Developer ID signature and notarization, then atomically replace the running binary. The download is staged in a private (0700) directory, and the final hash, `chmod` and identity check go through one descriptor opened without following symlinks, so no other account can swap the file between the checks and the install. A process running as the same user is outside this boundary: it could replace the installed binary directly. Everything project-specific comes in through `SelfUpdate.Configuration`. |
