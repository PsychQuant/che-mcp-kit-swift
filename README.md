...
|---|---|
| `SelfUpdate` | `--self-update`: fetch the latest GitHub release, check its SHA-256 companion, verify the Developer ID signature and notarization, then atomically replace the running binary. The download is written as a new file in a private staging directory (0700; inherited ACL entries removed where the volume supports ACLs, and the update refuses if they cannot be), opened once without following symlinks, hashed and `chmod`-ed through that descriptor, and must still be the same file after the signature check and before `rename(2)`. That keeps out accounts with no write access to the install directory and accounts that can only add entries to it (sticky `+t`). The same user, and any account that can remove or rename entries in the install directory, are outside this boundary: they could replace the installed binary directly. Everything project-specific comes in through `SelfUpdate.Configuration`. |
